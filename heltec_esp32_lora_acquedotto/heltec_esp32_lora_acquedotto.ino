 ...
 *
 * Revisione: rev008 — OLED pagina sistema: mostra IP WiFi
 */

#define SKETCH_REV 8
#define SKETCH_REV_STR "rev008"
...
void disegnaPaginaOled(uint8_t pagina) {
  ...
  if (OLED_NUM_PAGINE == 0 || OLED_CAMBIO_PAGINA_MS == 0) {
    ...
    if (ultimoLivelloCm > 0.0f) {
      snprintf(l4, sizeof(l4), "LIV %.1f CM", ultimoLivelloCm);
    } else {
      snprintf(l4, sizeof(l4), "LIV --- CM");
    }
    if (ultimoCodiceThingSpeak == 200) {
      snprintf(l5, sizeof(l5), "TS OK  N:%d", campioniValidi);
    } else if (ultimoCodiceThingSpeak != 0) {
      snprintf(l5, sizeof(l5), "TS ERR %d", ultimoCodiceThingSpeak);
    } else {
      snprintf(l5, sizeof(l5), "N:%d %s", campioniValidi, oledStatoCloud);
    }
    oledShowLinesCompact(l1, l2, l3, l4, l5, "");
    return;
  }

  switch (pagina) {
    case 0:
      snprintf(l1, sizeof(l1), "SISTEMA 1/%d", OLED_NUM_PAGINE);
